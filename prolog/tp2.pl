...
fila(0, []).
fila(Columnas, Fila) :- Columnas > 0, Columnas2 is Columnas - 1, append([_], Fila2, Fila), fila(Columnas2, Fila2).
tablero(0,0,[]).
tablero(0,Columnas,[Fila]) :- Columnas > 0, fila(Columnas, Fila).
tablero(Filas, Columnas, Tablero) :- Filas > 0, F is Filas - 1, fila(Columnas, Fila), append([Fila], RestoDeFilas, Tablero), tablero(F, Columnas, RestoDeFilas).

%% Ejercicio 2
%% ocupar(+Pos,?Tablero) será verdadero cuando la posición indicada esté ocupada.
% ocupar(pos(0, X), T) :- ocupar
% ocupar(_,_).
